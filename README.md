...
├── features.py    # 正規化、補幀、between_hits_windows —— 與訓練完全一致的幾何前處理
...
windows = between_hits_windows(hits, len(features), fps)     # 或任何 [start, end) 清單
probs = predict_windows(model, features, windows, device="cuda")
```

兩個階段走同一個 `predict_windows`，差別只在**窗的清單**。但那兩種窗**不是對等的東西**，所以只有一種住在 common：

- **`between_hits_windows`（在 common）**：`between_2_hits_with_max_limits`，權重檔名裡就寫著它。這是**訓練時的切法**，
  屬於權重的契約，不是任何階段的意見——所以它跟模型放在一起，哪個階段改它都是錯的。
- **`scan_windows`（在 event_detection）**：「每一幀一個窗」是這個階段自己的**策略**（它還沒有擊球點，正是要去找），
  `half` 和 `stride` 是它的旋鈕。旋鈕該跟有意見的程式碼放在一起，而不是擺在一個**永遠不准調**的切窗法旁邊——
  並排會讓人以為兩者一樣可調，那正是最危險的誤解。

窗切好之後怎麼把機率融合成擊球事件（side 融合、鎖定區、dead-time），同理是**各階段自己的啟發式**，不放進 common。
...
輸出 `strokes.json`：`events.json` 裡的**每一拍**對應一筆，順序相同（`event_index` 就是它在 `events.json` 的位置）。
每筆記錄球種（8 類）、**擊球者**（`top`/`bottom`）、信心，以及所屬 segment。

**擊球者在這裡才被寫下來**。`event_detection` 刻意不寫（見上節）——BST 的 25 類頭一次 forward 就同時回答
「哪一種球」和「誰打的」，所以整條 pipeline 只有這個階段有資格宣稱它。

跟隔壁的 `event_detection` 剛好相反：**單相位、沒有 cache、沒有任何門檻**。BST 對每一拍只跑一次（一整場約 900–1000
個窗，GPU 上 2 秒），而且 argmax 就是答案，沒有東西可調——存機率沒有意義，調參也無從調起。