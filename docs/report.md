...
# 四、實驗結果
## 資料集與重現方式
本章各模組共用同一套影片庫。資料集由 16 支 YouTube 羽球單打完整轉播構成，涵蓋 2018 至 2023 年的國際賽事。影片皆為 1920 × 1080，幀率為 25 或 30 fps，編碼格式包含 H.264、AV1 與 VP9。全部影片合計 1,704,001 個影格、約 18.09 小時。為避免同一場比賽的資料同時出現在開發與評估中，資料以影片為單位切分為 7 場開發集與 9 場測試集；開發集用於設計與調整規則，測試集不參與開發，僅在完成後用於報告測試結果。由於其中 15 支影片來自 BWF 官方製作，跨轉播單位的泛化能力仍待後續驗證。
...
為使影格編號可被他人重現，`experiments/dataset/videos.json` 與 `videos.md` 記錄每支影片的 YouTube 來源、解析度、幀率、影格數、編碼、SHA-256 雜湊及影格指紋。下載影片後，應先放至 `matches/<代號>/input/`，並執行下列指令確認檔案與標記時所用影片一致：
...
影片對照資料及實驗程式均置於 `experiments/`，因此本章後續數字可由相同影片與版本化程式重新取得。
## 各模組結果
   （這裡放一張「全系統摘要表」，一列一模組、只放頭條指標）
### 賽事影片分段

在 9 場未參與開發的比賽、670 個真值片段上，本模組在 IoU ≥ 0.9 下達到 Precision 98.37%、Recall 98.96%，邊界誤差中位數為 0 影格。16 場、1354 個片段的整體 F1 為 99.01%，其中 11 場零錯誤。分段結果會直接決定下游分析範圍，因此本節分別驗證片段偵測、邊界精度，並分析所有失配案例。

真值、標記程式、評分程式與逐場結果皆公開於 `experiments/match_segmentation/`。

本模組的真值定義為「固定主機位之連續比賽畫面」的起訖影格，起訖兩端均包含。所有真值由同一位標記者完成。

#### 偵測準確率

配對規則為 IoU ≥ 0.9、一對一，未配對的輸出段計為 FP、未配對的真值段計為 FN。

| 比賽       | 真值 ans | 輸出 prog |  matched |     FP |     FN | Precision |    Recall |        F1 |
| ---------- | -------: | --------: | -------: | -----: | -----: | --------: | --------: | --------: |
| **開發集** |  **684** |   **687** |  **681** |  **6** |  **3** | **99.13** | **99.56** | **99.34** |
| **測試集** |  **670** |   **674** |  **663** | **11** |  **7** | **98.37** | **98.96** | **98.66** |
...
| ---------- | ---------: | -------: | -----------: | ---------: | -------: | -----------: |
| **開發集** |   **+0.0** | **0.31** |       **+6** |   **+0.0** | **0.01** |       **−3** |
| **測試集** |   **+0.0** | **0.19** |       **+4** |   **+0.0** | **0.05** |      **−10** |
| **合計**   |   **+0.0** | **0.26** |       **+6** |   **+0.0** | **0.03** |      **−10** |

1344 個配對段中沒有任何一段的邊界誤差超過 10 影格。只要一個回合被完整切出來，它的邊界幾乎一定是準的。

#### 失敗案例與限制

27 筆失配可歸為三類，且各自集中於特定影片。其中漏掉表示回合未被完整切出，多切則表示產生額外片段。

| 成因                             |   多切 |   漏掉 | 影響的比賽                           |
| -------------------------------- | -----: | -----: | ------------------------------------ |
| 一、播報圖卡動畫把回合切成碎片   |     11 |      6 | TTY_vs_ASY_2023、test1、test2、test3 |
| 二、全域亮度變化使跨片段篩選失效 |      1 |      2 | test4                                |
| 三、門檻邊緣的高強度回合被切斷   |      5 |      2 | test6                                |
| **合計**                         | **17** | **10** |                                      |

**一、播報圖卡動畫。** 轉播在回合開始或結束時播放球員名條、比分卡等動畫，使 MAD 超過門檻，將回合切成碎片。動畫持續 3–6 秒，遠超過邊界精修可橋接的 12 影格，因此無法重新連接。

**二、全域亮度變化。** test4 開賽後前兩個回合整段消失。MAD 中位數與正常回合相近，表示門檻判斷本身沒有問題。真正失效的是跨片段一致性篩選。由於開賽初期場館燈光逐漸變亮，兩段畫面與其他回合雖然構圖相同，整體像素差卻明顯較高，因而被誤判為非比賽畫面。此類錯誤表現為整段消失，而非邊界偏移。

**三、門檻邊緣的高強度回合。** test6 有兩個高強度回合被切成數段，其中一個 11.4 秒的回合被切成三段。切斷處與保留區段的 MAD 均接近門檻，顯示球員劇烈移動使畫面差異在門檻附近波動。由於目前採用全片固定門檻，無法適應此類時間上的動態變化。

**已知但未在本資料集上觸發的限制。** 若影片包含大量且相似的非比賽畫面，跨片段一致性篩選可能失效。系統亦假設存在穩定主機位，不適用於手持或多機快切影片。此外，16 支影片中有 15 支來自 BWF 官方製作，因此跨轉播單位的泛化能力尚未驗證。
