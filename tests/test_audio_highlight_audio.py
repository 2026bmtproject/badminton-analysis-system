...
import modules.audio_highlight.audio as audio_module
from modules.audio_highlight.audio import (
    AudioRangeError,
...
from modules.audio_highlight.windows import AnalysisWindow
...
def test_ffmpeg_normalization_runs_once_then_reuses_cache(
    ...
    normalizer.normalize(media, cache).close()
    assert len(calls) == 2
    Path(f"{cache}.json").write_text("broken metadata", encoding="utf-8")
    normalizer.normalize(media, cache).close()
    assert len(calls) == 3
