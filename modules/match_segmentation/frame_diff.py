...
def compute_frame_diff(
    ...
    if not fps or fps <= 0:
        fps = DEFAULT_FPS
        console.warn(
            f"cannot read fps from {video_path} - assuming {DEFAULT_FPS:g}.\n"
            "If the source is not 30 fps, every timestamp in segments.json\n"
            "and every downstream window length will be off by that ratio."
        )
