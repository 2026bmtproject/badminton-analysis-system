...
from modules.audio_highlight.audio import AudioWindow, MONO_CHANNELS, YAMNET_SAMPLE_RATE_HZ
...
def configure_hub_cache() -> Path:
    """Keep TFHub SavedModel paths ASCII-safe, including on Windows."""
    configured = os.environ.get("TFHUB_CACHE_DIR")
    candidates = [Path(configured)] if configured else [
        Path(tempfile.gettempdir()) / "badminton-audio-highlight-tfhub",
        Path.home() / ".cache" / "badminton-audio-highlight-tfhub",
    ]
    for candidate in candidates:
        ...
        try:
            ...
            continue
        os.environ["TFHUB_CACHE_DIR"] = str(resolved)
        return resolved
    raise YamNetError(
        "YAMNet needs a writable ASCII-only cache path. Set TFHUB_CACHE_DIR "
        "to a directory outside any non-ASCII workspace or user-profile path."
    )
