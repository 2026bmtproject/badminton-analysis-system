...
from modules.commentary.adapters.upstream import (
    CourtPositionToPlayer,
    UpstreamStageData,
    build_rally_fact_from_stages,
)
from modules.commentary.adapters.vision import (
    CourtCalibration,
    CourtPosition,
    PoseFrame,
    ShuttlePoint,
    CourtCalibrationPolicy,
)
from modules.common.court_geometry import COURT_LENGTH_M, COURT_WIDTH_M
from modules.commentary.schemas import Player, RallyFactEvent
...
def _usable_point(
    ...
    if not isinstance(point, (list, tuple)) or len(point) != 3 or not all(isinstance(v, (int, float)) and math.isfinite(v) for v in point):
        return None
    x, y, confidence = point
    return (x, y) if confidence >= threshold else None


def _usable_bbox(pose: PoseFrame) -> bool:
    box = pose.bbox
...
def _pose_fact(
    *,
    ...
    pose: PoseFrame,
    threshold: float,
) -> CompactPoseFact | None:
    if (pose.keypoints is None or len(pose.keypoints) != 17
            or any(not isinstance(p, (list, tuple)) or len(p) != 3 or not all(isinstance(v, (int, float)) and math.isfinite(v) for v in p)
                   or p[2] < 0 for p in pose.keypoints)):
        return None
    ...
    if not usable_count or (hip_center is None and not _usable_bbox(pose)):
        return None
    body_center = hip_center if hip_center is not None else (
    ...
    return CompactPoseFact(
        fact_id=(
            f"rally:{segment_index}:stroke:{event.event_index}:pose"
        ),
...
def _inverse_3x3(matrix: list[list[float]]) -> list[list[float]] | None:
    if (not isinstance(matrix, list) or len(matrix) != 3
            or any(not isinstance(row, list) or len(row) != 3
                   or not all(isinstance(v, (int, float)) and math.isfinite(v) for v in row) for row in matrix)):
        return None
    a, b, c = matrix[0]
    d, e, f = matrix[1]
    g, h, i = matrix[2]
...
def _court_fact(
    *,
    ...
    return CompactCourtPositionFact(
        fact_id=f"rally:{segment_index}:stroke:{event.event_index}:court",
        source_frame=pose.frame,
        quality=quality,
        ...
        limitations=[*limitations, "observed_position_not_continuous_trajectory", "displacement_not_running_path"],
    )
...
def _shuttle_fact(
    *,
    ...
    if any(p.confidence is None for p in window):
        limitations.append("shuttle_confidence_unknown_observations_excluded")
    if quality != "reliable":
        limitations.append("shuttle_path_quality_is_not_reliable")
    return CompactShuttlePathFact(
        fact_id=(
            f"rally:{segment_index}:stroke:{event.event_index}:shuttle"
        ),
...
def _pose_candidates(
    ...
    """Order in-window records by distance, then earlier source frame."""
    frames = [item.frame for item in poses]
    left = bisect_left(frames, frame - max_delta)
    right = bisect_left(frames, frame + max_delta + 1)
    return sorted(poses[left:right], key=lambda p: (abs(p.frame - frame), p.frame))
...
def _nearest_pose(
    ...
    """Independent court input selection, including bbox-only observations."""
    nearby = _pose_candidates(poses, frame, max_delta)
    candidates = [p for p in nearby if any(_usable_point(p, i, threshold) is not None for i in range(17))]
    if not candidates:
        candidates = [p for p in nearby if _usable_bbox(p)]
    return min(candidates, key=lambda p: (abs(p.frame - frame), p.frame), default=None)
...
def _position_for_player(
    ...
    if player is None or mapping is None:
        return None
    return mapping.position_for(player)
...
def build_compact_rally_facts(
    *,
    ...
    warnings: list[str] = []
    pose_stage = stages.vision
    shuttle_stage = stages.vision
    if stages.vision is not None:
        warnings.extend(stages.vision.warnings)
    ...
    calibration, court_warning = _select_calibration(
        stages,
        segment_index,
        require_confirmation=CourtCalibrationPolicy(stages.court_calibration_policy) == CourtCalibrationPolicy.STRICT,
    )
    ...
    if pose_stage is not None:
        for pose in pose_stage.poses:
            if pose.player in poses_by_position:
                poses_by_position[pose.player].append(pose)
        for poses in poses_by_position.values():
            poses.sort(key=lambda item: item.frame)
    shuttle_method = stages.shuttle_method
    if shuttle_method is None:
        warnings.append("shuttle_method_unknown_no_direction")
    shuttle_points = sorted(
        (
            item
            for item in (shuttle_stage.points if shuttle_stage is not None else [])
            if shuttle_method is not None and item.method == shuttle_method
        ),
    ...
    for event in fact.events:
        ...
        raw_pose = (
            _nearest_pose(
                ...
                resolved_config.pose_keypoint_confidence,
            )
            if position is not None
            else None
        )
        pose_fact = None
        # A usable joint alone does not guarantee a constructible pose fact.
        # Keep court input selection independent of this geometry fallback.
        for candidate in _pose_candidates(
            poses_by_position[position] if position is not None else [],
            event.frame,
            resolved_config.pose_max_frame_delta,
        ):
            pose_fact = _pose_fact(
                segment_index=segment_index,
                event=event,
                pose=candidate,
                threshold=resolved_config.pose_keypoint_confidence,
            )
            if pose_fact is not None:
                break
        if pose_fact is None:
                ...
                update={
                    ...
                    "limitations": [
                        *court_fact.limitations,
                        "court_calibration_unconfirmed_allowed_by_policy",
                    ]
                }
            )
        ...
        shuttle_fact = (
            ...
            if shuttle_stage is not None and shuttle_method is not None
            else None
        )
        ...
        if shuttle_method is None:
            event_warnings.append("shuttle_method_unknown_no_direction")
    ...
    segment = stages.segments[segment_index]
