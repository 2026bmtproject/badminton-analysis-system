...
PATTERN_METADATA = {
    ...
    "stroke_diversity": (0.75, "觀察到至少三類不同球路"),
}
...
def analyze_stroke(
    ...
    band = (
        "reliable"
        if confidence is not None and confidence >= RELIABLE_CONFIDENCE
        else (
            "cautious"
            if confidence is not None and confidence >= CAUTIOUS_CONFIDENCE
            else "low"
        )
    )
    return AnalyzedStroke(
        ...
        salience=min(
            ...
            * (0.8 + 0.2 * confidence) if confidence is not None else 0.0,
            1.0,
        ),
    )
...
def analyze_rally(fact: RallyFact) -> RallyAnalysis:
    ...
    usable = [analyze_stroke(fact, event) for event in fact.events]
    reliable = [stroke for stroke in usable if stroke.confidence_band == "reliable" and stroke.player is not None and stroke.stroke_type is not None]
    ...
    warnings = ["sequence_candidates_only_no_intent_or_causality", "salience_is_not_correctness_probability"]
    if any(s.player is None or s.stroke_type is None for s in usable):
        warnings.append("unknown_stroke_observations_retained")
    if cautious:
        warnings.append("cautious_strokes_present")
    if any(s.confidence_band == "low" for s in usable):
        warnings.append("low_or_unknown_confidence_strokes_retained")
    if not usable:
